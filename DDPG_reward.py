...
def reward(taskIndex, plannerCurrNo, plannerCurrTime, plannerCurrPrevConsecutiveTime, time_left_episode, df,
    ...
    if missingTime > time_left_episode:
        ...
        if (time_left_episode - plannerCurrTime) > minTimeReq_anyPlanner:
            ...
            return np.array([-Epsilon + R_t], dtype=np.float32), False
        else:
            # no time left for other planner --> very bad
            return np.array([-Theta - Epsilon + R_t], dtype=np.float32), False
        ...
        if missingTime <= 0:
            ...
            return np.array([R_s + R_p + R_t], dtype=np.float32), True
        else:
            R_p = 0
            R_s = 0
            return np.array([R_s + R_p + R_t], dtype=np.float32), False
