...
from DDPG_reward import reward as ddpg_reward
...
class evaluation:
    def __init__(self, time_per_ep, t_SD, p_SD, w_and_h, nb_planners, omnicron, Theta, Epsilon):
        ...
        self.env = portfolio_environment.PortfolioEnvironment(self.testSet, self.taskFolderLoc, time_per_ep, dqn_reward,
                                                              ddpg_reward, t_SD=t_SD, p_SD=p_SD,
                                                              nb_planners=nb_planners, omnicron=omnicron, Theta=Theta,
                                                              Epsilon=Epsilon, time_per_ep=time_per_ep)
        self.resizer = resizer(w_and_h)

    @staticmethod
    def randAction(timeLeft, n_actions):
        return torch.tensor([[random.random() * n_actions]], dtype=torch.int), \
               torch.tensor([random.random() * timeLeft], dtype=torch.float32)

    def evaluateNetwork(self, episodeNumbers, averageRewards, currentEpisodeNumber, discrete_agent, continuous_agent,
                        randAverageReward, rand_bool=False, n_actions=17):
        print("start testing...")
        discrete_agent.set_eval()
        continuous_agent.set_eval()
        ...
        for task_i_idx in range(num_of_tests):
            ...
            while not final_state and not time_restriction:
                number_of_passes += 1
                if not rand_bool:
                    state_additional = torch.unsqueeze(state_additional, dim=0)
                    action = discrete_agent.policy_net(state, state_additional)
                    actionNo = action.max(1)[1].view(1, 1)
                    action_t = continuous_agent.actor(state, state_additional, actionNo)
                    print("actionTime(testing): ", action_t)
                    print("action number(testing): ", actionNo)
                else:
                ...
                obs, reward_tuple, final_state, time_restriction, _ = self.env.step(complete_action)
                print(final_state)
                ...
                reward_a, reward_t = reward_tuple
                if final_state:
                    rewardTotal += (reward_a + reward_t)
                    number_correct += 1
